...
from pytorchKT.models.utils import ScheduledOptim, NoamOpt
...
class Trainer:
    def __init__(
        ...
        self._opt = NoamOptimizer(
            model=model, lr=lr, model_size=d_model, warmup=warm_up_step_count
        )
    ...
    def train(self):
        ...
        while self.step < total_steps:
            ...
            print(
                f"Current best weight: {self.max_step}.pt, best auc: {self.max_auc:.4f}"
            )
    ...
    def _forward(self, batch):
        ...
        pred = (
            torch.sigmoid(output) >= self._threshold
        ).long()  # shape: (batch_size, 1)
    ...
    def _train(self, batch_iter, num_batches):
        ...
        print(
            f"[Train]     time: {training_time:.2f}, loss: {loss:.4f}, acc: {acc:.4f}, auc: {auc:.4f}"
        )
    ...
    def _test(self, name, batches):
        ...
        print(f"correct: {num_corrects}, total: {num_total}")
        print(
            f"[{name}]      time: {training_time:.2f}, loss: {loss:.4f}, acc: {acc:.4f}, auc: {auc:.4f}"
        )
