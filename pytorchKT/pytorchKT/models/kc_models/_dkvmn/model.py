...
class MODEL(nn.Module):
    def __init__(
        ...
        self.init_memory_key = nn.Linear(self.memory_size, self.memory_key_state_dim)
        nn.init.kaiming_normal_(self.init_memory_key)
        self.init_memory_value = nn.Linear(
        ...
        memory_value = nn.Parameter(
            torch.cat(
                [self.init_memory_value.unsqueeze(0) for _ in range(batch_size)], 0
            ).data
        )
    ...
    def forward(self, q_data, qa_data, target, student_id=None):
        batch_size, seqlen = q_data.shape[0], q_data.shape[1]
