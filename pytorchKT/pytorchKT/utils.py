...
def get_data_tl(data_path):
    ...
    with open(data_path, "r") as f:
        ...
        for i in range(num_of_users):
            ...
            assert (
                user_interaction_len == len(qid_list) == len(is_correct_list)
            ), "length is different"
...
def get_data_user_sep(data_path):
    ...
    for user_path in user_path_list:
        with open(os.path.join(data_path, user_path), "rb") as f:
            lines = f.readlines()
            lines = lines[1:]
            num_of_interactions = len(lines)
            for end_index in range(num_of_interactions):
                sample_infos.append((os.path.join(data_path, user_path), end_index))
    return sample_infos, num_of_users
