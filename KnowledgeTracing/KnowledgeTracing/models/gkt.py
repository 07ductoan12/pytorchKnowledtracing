...
class GKT(Module):
    ...
    def aggregate(self, xt_emb, qt_onehot, q_emb, ht):
        ...
        ht_ = qt_onehot * torch.cat([ht, xt_emb], dim=-1) + (1 - qt_onehot) * torch.cat(
            [ht, q_emb], dim=-1
        )
...
class PAM(GKT):
    ...
    def f_neighbor(self, ht_, qt):
        ...
        src = ht_
        tgt = torch.gather(
            ht_, dim=1, index=qt.unsqueeze(-1).unsqueeze(-1).repeat(1, 1, ht_.shape[-1])
        )
...
class MHA(GKT):
    def __init__(self, num_q, hidden_size, num_attn_heads, method):
        ...
        self.Q = Linear(self.hidden_size * 2, self.hidden_size, bias=False)
        self.K = Linear(self.hidden_size * 2, self.hidden_size, bias=False)
        self.V = Linear(self.hidden_size * 4, self.hidden_size, bias=False)
        #############################################################
    ...
    def f_neighbor(self, ht_, qt):
        ...
        q = self.Q(tgt)
        k = self.K(src)
        v = self.V(torch.cat([tgt.repeat(1, self.num_q, 1), src], dim=-1))
