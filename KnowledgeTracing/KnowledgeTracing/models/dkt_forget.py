...
class DKTForget(Module):
    def __init__(
        ...
        self.hidden_size = emd_size
        self.emb_type = emb_type

        if emb_type.startwith("qid"):
    ...
    def forward(self, q, r, dgaps):
        ...
        theta_out = self.c_integration(
            h,
            dgaps["shft_rgaps"].long(),
            dgaps["shft_sgaps"].long(),
            dgaps["shft_pcounts"].long(),
        )
...
class CIntegration(Module):
    def __init__(self, num_rgap, num_sgap, num_pcount, emb_dim) -> None:
        ...
        print(
            f"num_sgap: {num_sgap}, num_rgap: {num_rgap}, num_pcount: {num_pcount}, ntotal: {ntotal}"
        )

    def forward(self, vt, rgap, sgap, pcount):
        rgap, sgap, pcount = (
            self.rgap_eye[rgap].to(device),
            self.sgap_eye[sgap].to(device),
            self.pcount_eye[pcount].to(device),
        )
