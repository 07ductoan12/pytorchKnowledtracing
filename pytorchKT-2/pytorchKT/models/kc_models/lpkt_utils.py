...
def predict_one_step(
    model, data, return_details=False, process=False, return_raw=False
):
...
def _parser_row(row, data_config, ob_portions=0.5):
    ...
    if times != []:
        ...
        it_times = np.maximum(
            np.minimum((np.array(times) - np.array(shft_times)) // 1000 // 60, 43200),
            -1,
        )
...
def _evaluate_multi_ahead_accumulative(
    model, data_config, batch_size=1, ob_portions=0.5, acc_threshold=0.5, max_len=200
):
    ...
    for i, row in df.iterrows():
        (
            ...
            seq_len,
            start_index,
        ) = _parser_row(row, data_config=data_config, ob_portions=ob_portions)
        if i % 10 == 0:
    ...
    acc = metrics.accuracy_score(
        y_true_list, [1 if p >= acc_threshold else 0 for p in y_pred_list]
    )

    return auc, acc


def _evaluate_multi_ahead_help(
    model, data_config, batch_size, ob_portions=0.5, acc_threshold=0.5
):
    ...
    for i, row in df.iterrows():
        ...
        for data in dataloader:
            ...
            y = (
                predict_one_step(model, data, process=False)[:, -1]
                .detach()
                .cpu()
                .numpy()
                .flatten()
...
def lpkt_evaluate_multi_ahead(
    ...
    with torch.no_grad():
        if accumulative:
            ...
            auc, acc = _evaluate_multi_ahead_accumulative(
                model,
                data_config,
                batch_size=batch_size,
                ob_portions=ob_portions,
                acc_threshold=acc_threshold,
                max_len=max_len,
            )
        else:
            print("predict use no accumulative")
            auc, acc = _evaluate_multi_ahead_help(
                model,
                data_config,
                batch_size=batch_size,
                ob_portions=ob_portions,
