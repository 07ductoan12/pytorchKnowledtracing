...
class LPKT(nn.Module):
    ...
    def forward(self, e_data, a_data, it_data=None, at_data=None, qtest=False):
        ...
        h_pre = (
            nn.init.xavier_uniform_(torch.zeros(self.n_question + 1, self.d_k))
        ...
        for t in range(0, seq_len - 1):
            ...
            c_tilde = torch.unsqueeze(
                torch.sum(
                    torch.squeeze(
                        self.q_matrix[e_data[:, t + 1]].view(batch_size, 1, -1), dim=1
                    ),
                    1,
                ),
                -1,
            ...
            h_tilde = (
                self.q_matrix[e_data[:, t + 1]]
                .view(batch_size, 1, -1)
                .bmm(h)
                .view(batch_size, self.d_k)
                / c_tilde
            )
            # print(f"h_tilde: {h_tilde.shape}")
            y = (
                self.sig(
                    self.linear_5(torch.cat((e_embed_data[:, t + 1], h_tilde), 1))
                ).sum(1)
                / self.d_k
            )
            pred[:, t + 1] = y
