...
class Blocks(Module):
    ...
    def forward(
        self, q: torch.Tensor = None, k: torch.Tensor = None, v: torch.Tensor = None
    ):
        q, k, v = q.permute(1, 0, 2), k.permute(1, 0, 2), v.permute(1, 0, 2)
        causal_mask = ut_mask(seq_len=k.shape[0])
        attn_emb, _ = self.attn(q, k, v, attn_mask=causal_mask)
        attn_emb = self.attn_dropout(attn_emb)
        attn_emb, q = attn_emb.permute(1, 0, 2), q.permute(1, 2, 0)
...
class SAKT(Module):
    def __init__(
        ...
        if emb_type.startswith("qid"):
            ...
            self.excurcies_emb = Embedding(num_c, emb_size)
    ...
    def base_emb(self, q, r, qry):
        x = q + self.num_c * r
        qshftemb, xemb = self.excurcies_emb(qry), self.interaction_emb(x)
