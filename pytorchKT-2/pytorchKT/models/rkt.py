    ...
    for i in range(batch):
        ...
        for j in range(seqlen):
            for k in range(seqlen):
                if (
                    qseqs_cpu[i][j] in corr_dict.keys()
                    and qseqs_cpu[i][k] in corr_dict[qseqs_cpu[i][j]].keys()
                ):
...
def computeTime(time_seq, time_span, batch_size, size):
    ...
    time_matrix = torch.abs(
        torch.unsqueeze(time_seq, axis=1)
        .repeat(1, size, 1)
        .reshape((batch_size, size * size, 1))
        - torch.unsqueeze(time_seq, axis=-1)
        .repeat(
            1,
            1,
...
class MultiHeadedAttention(nn.Module):
    ...
    def forward(self, query, key, value, rel, l1, l2, timestamp, mask=None):
        ...
        query, key, value = [
            l(x)
            .view(batch_size, seq_length, self.num_heads, self.head_size)
            .transpose(1, 2)
            for l, x in zip(self.linear_layers, (query, key, value))
        ]

        # Apply attention
        out, self.prob_attn = attention(
            query, key, value, rel, l1, l2, timestamp, mask, self.dropout
        )
        out = (
            out.transpose(1, 2)
            .contiguous()
            .view(batch_size, seq_length, self.total_size)
        )
...
class RKT(nn.Module):
    def __init__(
        ...
        self.position_emb = CosinePositionalEmbedding(
            d_model=embed_size, max_len=seq_len
        )
    ...
    def forward(self, dcur, rel_dict, train=True):
        ...
        outputs, attn = self.attn_layers[0](
            query, inputs, inputs, rel, self.l1, self.l2, time, mask
        )
        outputs = self.dropout(outputs)

        for l in self.attn_layers[1:]:
            residual, attn = l(
                query, outputs, outputs, rel, self.l1, self.l2, time, mask
            )
