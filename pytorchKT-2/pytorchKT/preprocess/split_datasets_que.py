...
def split_question(
    dname, fname, dataset_name, configf, min_seq_len=3, maxlen=200, kfold=5
):
    """split main function
    ...
    ins, ss, qs, cs, seqnum = calStatistics(
        splitdf, stares, "original train+valid question level"
    )
    print(
    ...
    ins, ss, qs, cs, seqnum = calStatistics(
        split_seqs, stares, "train+valid sequences question level"
    )
    print(
    ...
    ins, ss, qs, cs, seqnum = calStatistics(
        test_df, stares, "test original question level"
    )
    print(
        f"original test interactions num: {ins}, select num: {ss}, qs: {qs}, cs: {cs}, seqnum: {seqnum}"
    )
    ins, ss, qs, cs, seqnum = calStatistics(
        test_seqs, stares, "test sequences question level"
    )
    print(
    ...
    ins, ss, qs, cs, seqnum = calStatistics(
        test_window_seqs, stares, "test window question level"
    )
    print(
