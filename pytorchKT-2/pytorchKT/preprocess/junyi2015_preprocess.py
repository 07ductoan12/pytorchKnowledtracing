...
def load_q2c(qname):
    df = pd.read_csv(qname, encoding="utf-8", low_memory=False).dropna(
        subset=["name", "topic"]
    )
    dq2c = dict()
...
def read_data_from_csv(read_file, write_file, dq2c):
    ...
    usedf["time_taken_attempts"] = (usedf["time_taken_attempts"].fillna(-100)).astype(
        str
    )  # only hint! False correct
    usedf.loc[:, "time_taken_attempts"] = (
        usedf["time_taken_attempts"]
        .astype(str)
        .apply(lambda x: int(x.split("&")[0]) * 1000)
        .astype(str)
    )
    ...
    for ui in ui_df:
        ...
        curdf["time_done"] = (
            curdf["time_done"].apply(lambda x: round(int(x) / 1000)).astype(str)
        )  # ms
