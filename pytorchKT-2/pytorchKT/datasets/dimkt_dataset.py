...
class DIMKTDataset(Dataset):
    def __init__(
        self, dpath, file_path, input_type, folds, qtest=False, diff_level=None
    ):
        ...
        if not os.path.exists(skills_difficult_path) or not os.path.exists(
            questions_difficult_path
        ):
        ...
        if not os.path.exists(processed_data):
            ...
            if self.qtest:
                self.dori, self.dqtest = self.__load_data__(
                    self.sequence_path,
                    skills_difficult_path,
                    questions_difficult_path,
                    folds,
        ...
        print(
            f"file path: {file_path}, qlen: {len(self.dori['qseqs'])}, clen: {len(self.dori['cseqs'])}, rlen: {len(self.dori['rseqs'])}, sdlen: {len(self.dori['sdseqs'])}, qdlen:{len(self.dori['qdseqs'])}"
        )
    ...
    def __load_data__(self, sequence_path, sds_path, qds_path, folds, pad_val=-1):
        ...
        with open(sds_path, "r", encoding="UTF8") as f:
            ...
            for idx, values in enumerate(sds_keys):
                sds[int(values)] = int(sds_vals[idx])
        ...
        with open(qds_path, "r", encoding="UTF8") as f:
            ...
            for idx, value in enumerate(qds_keys):
                qds[int(value)] = int(qds_vals[idx])
        ...
        for idx in dori:
            if idx not in ["rseqs"]:  # in ["smasks", "tseqs"]:
                dori[idx] = LongTensor(dori[idx])
            else:
                dori[idx] = FloatTensor(dori[idx])
        ...
        if self.qtest:
            for idx in dqtest:
                dqtest[idx] = LongTensor(dqtest[idx])[:, 1:]
...
def difficult_compute(df, sds_path, qds_path, diff_level):
    ...
    df2 = pd.DataFrame(
        {"concepts": concepts, "questions": questions, "responses": responses}
    )
